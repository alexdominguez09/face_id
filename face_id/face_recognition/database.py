...
class FaceDatabase:
    ...
    def face_exists(self, embedding: np.ndarray, threshold: float = 0.85) -> Optional[Dict]:
        ...
        Args:
            ...
            threshold: Similarity threshold to consider as duplicate (default 0.85, stricter than recognition)
        ...
        # Return best match if it exceeds threshold (considered a duplicate)
        if best_match and best_similarity >= threshold:
            logger.info(f"Duplicate face detected: {best_match['name']} (ID: {best_match['id']}, similarity: {best_similarity:.3f})")
            return best_match
